...
-- Criar índices únicos condicionais separadamente
CREATE UNIQUE INDEX idx_unique_student_shift_active 
ON public.computer_assignments (student_id, shift) 
WHERE active = true;

CREATE UNIQUE INDEX idx_unique_computer_shift_active 
ON public.computer_assignments (computer_number, shift) 
WHERE active = true;
