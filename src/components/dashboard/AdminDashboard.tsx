    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard
          ...
          trend={{ value: 0, label: "estudantes cadastrados", isPositive: true }}
        />
        <StatsCard
          title="Turmas Ativas"
          value={statsLoading ? "..." : stats.totalClasses}
          icon={<GraduationCap className="h-4 w-4" />}
          trend={{ value: 0, label: "turmas cadastradas", isPositive: true }}
        />
        <StatsCard
          title="Declarações Pendentes"
          value={statsLoading ? "..." : stats.pendingDeclarations}
          icon={<AlertTriangle className="h-4 w-4" />}
          trend={{ value: 0, label: "aguardando processamento", isPositive: false }}
        />
        <StatsCard
          title="Taxa de Frequência"
          value={statsLoading ? "..." : stats.attendanceRate}
          icon={<TrendingUp className="h-4 w-4" />}
          trend={{ value: 0, label: "frequência geral", isPositive: true }}
        />
      </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  {reportsLoading ? (
                    ...
                    <LineChart data={reportsData.attendanceByMonth}>
        ...
        <Card className="shadow-[var(--shadow-card)]">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              ) : reportsData.topAbsentStudents.length > 0 ? (
                reportsData.topAbsentStudents.slice(0, 4).map((student, index) => (
          ...
          <CardContent>
            <div className="space-y-4">
              {reportsLoading ? (
                ...
                reportsData.classDistribution.map((cls, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
